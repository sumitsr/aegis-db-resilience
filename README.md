...
Aegis uses the ANSI/ISO standard codes as a baseline, ensuring high portability across most major DBMS (Postgres, Oracle, SQL Server, etc.). It also includes vendor-specific extensions (marked with 🐘 for Postgres) to provide granular insights.

| SQLState code | Origin | Classification | Description |
| :--- | :--- | :--- | :--- |
| `08xxx` | **ANSI** | `UNAVAILABLE` | Connection/Network failure |
| `23505` | **ANSI** | `INTEGRITY_UNIQUE` | Unique constraint violation |
| `23503` | **ANSI** | `INTEGRITY_FK` | Foreign key violation |
| `23502` | **ANSI** | `INTEGRITY_NOT_NULL` | Missing mandatory value |
| `23514` | **ANSI** | `INTEGRITY_CHECK` | Check constraint failure |
| `23P01` | 🐘 **PG** | `INTEGRITY_EXCLUSION` | Exclusion constraint violation |
| `23xxx` | **ANSI** | `INTEGRITY_GENERIC` | Other integrity fault |
| `40001` | **ANSI** | `TRANSIENT` | Serialization failure (retryable) |
| `40P01` | 🐘 **PG** | `TRANSIENT` | Deadlock detected (retryable) |
| `55P03` | 🐘 **PG** | `TRANSIENT` | Lock not available (retryable) |
| `57014` | 🐘 **PG** | `TIMEOUT` | Query cancelled / timeout |

> [!TIP]
> **Portability Note**: While codes starting with `23`, `08`, and `40` are highly standardized, some vendors use variations (e.g., Oracle often uses `23000` for unique violations). Aegis handles this by using a multi-stage classification: if a specific SQLState isn't matched, it falls back to Spring's `DataAccessException` hierarchy and then to JPA/Hibernate provider hints.
